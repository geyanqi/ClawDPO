...
| [项目动机](motivation.md) | 为什么以数据为核心产物，以及 rollout、DPO 和 Codex-as-Critic 各自解决什么问题 |
| [数据价值证据](data-value.md) | 为什么 rollout 和 Preference Pair 值得训练，以及用哪些记录支撑判断 |
...
- `design.md` 是行为、阈值和流程的唯一规范；实现与运行 prompt 必须与它同步，
  但不能再形成另一份设计说明。
